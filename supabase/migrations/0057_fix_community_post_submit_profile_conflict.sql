-- Linted 0057: fix community post submit profile bootstrap conflict target.
-- The RPC returns a column named id, so ON CONFLICT (id) is ambiguous inside
-- PL/pgSQL. Target the profiles primary key constraint explicitly.
