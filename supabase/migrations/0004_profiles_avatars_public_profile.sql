...
alter table public.profiles
  ...
  add column if not exists resume_highlight_id uuid;
...
begin
  if not exists (
    ...
    alter table public.profiles
      add constraint profiles_resume_highlight_id_fkey
      foreign key (resume_highlight_id) references public.resumes(id) on delete set null
      not valid;
  end if;
end $$;
