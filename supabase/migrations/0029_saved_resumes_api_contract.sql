-- Linted 0029: saved resumes API contract repair.
-- Reasserts the saved-resume table, RLS policies, grants, and PostgREST schema
-- reload so production cannot drift into a "table exists but API cannot use it"
-- state.
...
revoke all on table public.saved_resumes from anon;
grant select, insert, delete on table public.saved_resumes to authenticated;
