...
describe("comment media upload validation", () => {
	...
	it("accepts supported uploads when the browser omits the declared MIME type", () => {
		expect(
			getCommentImageUploadIssue({
				bytes: pngBytes,
				name: "screen.png",
	...
	it("rejects GIF, scriptable SVG, and spoofed image uploads", () => {
		expect(
			getCommentImageUploadIssue({
				bytes: gifBytes,
				name: "clip.gif",
	...
	it("enforces the 2MB size limit", () => {
		...
		).toBe("Keep comment images under 2MB.");
	});
});
